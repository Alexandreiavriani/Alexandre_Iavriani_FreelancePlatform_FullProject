...
<div id="sendWorkImg" >
  <div class="img_overlay2">
    <h1 >გამოგზავნილი სამუშაოები.  <br> <span class="text"> ამ გვერდზე გამოჩნდება ფრილანსრერის გამოგზავნილი სამუშაო.ფაილის ნახვისთვის უნდა გადაიხადოთ</span> </h1> 
      </div>
...
$result = mysqli_query($conn,"SELECT * from send_job inner join add_job on send_job.add_job_id=add_job.add_job_id inner join user
on send_job.user_id=user.user_id WHERE add_job.add_job_id='$job_id'");
...
<table class="table" style="width:1255px; margin-left:270px; margin-top:100px; table-layout: fixed;  ">
  ...
  <td> <a id="myLink" href="pay.php?post=<?php echo $job_id."&file=".$row['file']."&userName=".
  $row['name']."&userSurname=".$row['surname']."&id=".$user_id."&send_job_id=".$row['send_job_id'].
  "&title=".$row['title']."&customer_id=".$row['user_id']."&customer_name=".$row['customer_name']."&work_title=".$row['title'] ?>">  <button data-toggle="modal" data-target="#exampleModalCenter"     class="btn btn-primary"  >გადახდა</button></td></a>
      ...
      <td>ეს არის ჩემი გამოგზავილი სამუშაო!</td>
    </tr>
