...
<?php $user=$_GET['user'];?>
  ...
  if(isset($_GET['id'])){
    $main_id=$_GET['id'];
    $sql_update=mysqli_query($conn,"UPDATE pay_notif SET status=1 where pay_notif_id='$main_id'");
}
...
<table style='width:1100px; margin-top:100px;margin-left:285px;' class="table">
  <thead class="thead-dark">
    <tr>
      ...
      <th scope="col">გამოგზავნილი ფაილი</th>
      <th scope="col">სამუშაოს დასახელება</th>
      <th scope="col">შეფასება</th>
  ...
  <tbody>
    ...
    $sql_get1=mysqli_query($conn,"SELECT * from pay_notif  WHERE status=1 and pay_notif.user_id=$user_id");
    while($main_result1=mysqli_fetch_assoc($sql_get1)) {
    ?>
    
    
    <tr>
      <th scope="row"><?php echo $sr_no++;?></th>
      <td><?php echo $main_result1['name'];?></td>
      <td><?php echo $download="<a href='download.php?pay_notif_id=".$main_result1['pay_notif_id']."'>".$main_result1['file']." </a>";?></td>
      <td><?php echo $main_result1['title'];?></td>
      <td><a href="reviews.php?freelancer_name=<?php echo $main_result1['name'].'&freelancer_surname='.$main_result1['surname'].'&user='.$fetch['user_id']?>"><button class="btn btn-primary" >გადასვლა</button></a></td>
      <td><a href="deletes/read_pay_notif_delete.php?id=<?php echo $main_result1['pay_notif_id']?>"><i class="fa fa-trash" aria-hidden="true"></i></a></td>
